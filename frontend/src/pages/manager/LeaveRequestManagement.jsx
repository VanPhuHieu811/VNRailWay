...
//                       <button
//                         onClick={() => setReplacementModal({ open: true, request: req, mode: 'dirty' })}
...
//                       >
//                         <User className="w-4 h-4" /> Duyệt (Dirty Read)
//                       </button>

//                       <button
//                         onClick={() => setReplacementModal({ open: true, request: req, mode: 'fixedDirty' })}
//                         className="px-3 py-2 bg-blue-600 text-white rounded-lg
//                                   hover:bg-blue-700 transition-colors
//                                   flex items-center gap-2 shadow-sm text-sm whitespace-nowrap"
//                       >
//                         <User className="w-4 h-4" /> Duyệt (Fixed Dirty Read)
//                       </button>
...
// Import các service vừa cập nhật
import { 
    getLeaveRequests, 
    getAvailableStaff, 
    rejectLeaveRequest,
    // Import API Concurrency
    approveLeaveRequestLost, 
    approveLeaveRequestFixed,
    approveLeaveRequestDirtyRead,      // <--- Mới
    approveLeaveRequestFixedDirtyRead  // <--- Mới
...
const LeaveRequestManagement = () => {
  ...
  // mode: 'lost' | 'fixed' | 'dirty' | 'fixedDirty'
  const [replacementModal, setReplacementModal] = useState({ open: false, request: null, mode: 'lost' }); 
  const [rejectModal, setRejectModal] = useState({ open: false, request: null });
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      // Giả sử API này lấy danh sách đơn
      const data = await getLeaveRequests(activeTab);
      // Map dữ liệu cho khớp UI
      const mapped = data.map(item => ({
        ...item,
        trainCode: item.tripCode || item.trainCode, 
      }));
      setRequests(mapped);
    } catch (err) {
  ...
  // --- FETCH STAFF CHO MODAL ---
  useEffect(() => {
    if (replacementModal.open && replacementModal.request) {
      const fetchStaff = async () => {
        setLoadingStaff(true);
        try {
          // Gọi API lấy danh sách nhân viên rảnh
          const staff = await getAvailableStaff(
            replacementModal.request.trainCode,
            replacementModal.request.role
          );
          setAvailableStaffList(staff || []); // Fallback mảng rỗng
        } catch (error) {
          console.error("Error fetching staff", error);
          alert("Lỗi khi tìm nhân viên thay thế");
        } finally {
  ...
  const handleApprove = async () => {
    ...
    setLoadingApprove(true);
    try {
      const requestId = replacementModal.request.id;
      const staffId = selectedReplacement;

      // === LOGIC SWITCH API DỰA TRÊN CHẾ ĐỘ ===
      switch (replacementModal.mode) {
        // Demo Dirty Read
        case 'dirty':
            console.log("Demo: Dirty Read Mode");
            await approveLeaveRequestDirtyRead(requestId, staffId);
            break;
        
        case 'fixedDirty':
            console.log("Demo: Fixed Dirty Read Mode");
            await approveLeaveRequestFixedDirtyRead(requestId, staffId);
            break;

        // Demo Lost Update (Logic cũ của bạn)
        case 'lost':
            console.log("Demo: Lost Update Mode");
            await approveLeaveRequestLost(requestId, staffId);
            break;

        case 'fixed':
            console.log("Demo: Fixed Lost Update Mode");
            await approveLeaveRequestFixed(requestId, staffId);
            break;
      ...
      // Nếu thành công:
      alert(`Đã duyệt đơn thành công! (Chế độ: ${replacementModal.mode})`);
      
      // Cập nhật lại danh sách (xóa đơn đã duyệt khỏi list Pending)
  ...
  // Helper hiển thị tên chế độ trong Modal
  const getModeTitle = () => {
      switch(replacementModal.mode) {
          case 'dirty': return 'Demo Lỗi Dirty Read';
          case 'fixedDirty': return 'Xử lý An toàn (Fixed Dirty Read)';
          case 'lost': return 'Demo Lỗi Lost Update';
          case 'fixed': return 'Xử lý An toàn (Fixed Lost Update)';
          default: return 'Duyệt đơn';
      }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Quản lý Đơn nghỉ phép</h1>
          <p className="text-gray-500 text-sm">Duyệt đơn và phân công nhân sự (Demo Transaction)</p>
        </div>
      </div>

      <div className="flex gap-4 border-b border-gray-200 mb-6">
        <button onClick={() => setActiveTab('pending')} className={`pb-3 px-4 text-sm font-medium transition-colors border-b-2 ${activeTab === 'pending' ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-700'}`}>Chờ duyệt</button>
        <button onClick={() => setActiveTab('history')} className={`pb-3 px-4 text-sm font-medium transition-colors border-b-2 ${activeTab === 'history' ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-700'}`}>Lịch sử</button>
      </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  {activeTab === 'pending' && (
                    <div className="flex flex-col gap-2 shrink-0 min-w-[200px]">
                      ...
                      {/* Nút Demo Lost Update */}
                      <div className="flex gap-2">
                          <button onClick={() => setReplacementModal({ open: true, request: req, mode: 'lost' })} className="flex-1 px-3 py-2 bg-orange-100 text-orange-700 rounded hover:bg-orange-200 text-xs font-medium border border-orange-200">
                            Lost Update
                          </button>
                          <button onClick={() => setReplacementModal({ open: true, request: req, mode: 'fixed' })} className="flex-1 px-3 py-2 bg-green-100 text-green-700 rounded hover:bg-green-200 text-xs font-medium border border-green-200">
                            Fix Lost
                          </button>
                      </div>
                      ...
                      {/* Nút Demo Dirty Read */}
                      <div className="flex gap-2">
                          <button onClick={() => setReplacementModal({ open: true, request: req, mode: 'dirty' })} className="flex-1 px-3 py-2 bg-purple-100 text-purple-700 rounded hover:bg-purple-200 text-xs font-medium border border-purple-200">
                            Dirty Read
                          </button>
                          <button onClick={() => setReplacementModal({ open: true, request: req, mode: 'fixedDirty' })} className="flex-1 px-3 py-2 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 text-xs font-medium border border-blue-200">
                            Fix Dirty
                          </button>
                      </div>

                      <button onClick={() => setRejectModal({ open: true, request: req })} className="w-full px-3 py-2 border border-red-200 bg-red-50 text-red-600 rounded hover:bg-red-100 text-xs font-medium flex items-center justify-center gap-1">
      ...
      {/* --- MODAL CHỌN NGƯỜI THAY THẾ --- */}
      {replacementModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in">
            <div className="flex justify-between items-center p-5 border-b border-gray-100">
              <div>
                <h3 className="text-lg font-bold text-gray-800">Duyệt & Chọn người thay thế</h3>
                <p className="text-sm font-medium text-blue-600 mt-1">{getModeTitle()}</p>
              </div>
              <button onClick={() => setReplacementModal({ open: false, request: null, mode: 'lost' })} className="text-gray-400 hover:text-gray-600"><X className="w-6 h-6" /></button>
            </div>

            <div className="p-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Chọn nhân viên thay thế (Bắt buộc)</label>
              {loadingStaff ? (
                <div className="p-4 text-center text-gray-500 bg-gray-50 rounded-lg flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin"/> Đang tải danh sách...</div>
              ) : (
                <select className="w-full border border-gray-300 rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={selectedReplacement} onChange={(e) => setSelectedReplacement(e.target.value)}>
                  <option value="">-- Chọn nhân viên --</option>
                  {availableStaffList.map(staff => (
                    <option key={staff.MaNV} value={staff.MaNV}>{staff.HoTen} ({staff.MaNV}) - {staff.LoaiNhanVien}</option>
                  ))}
                </select>
              )}
            </div>
            ...
            <div className="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 rounded-b-xl">
              <button onClick={() => setReplacementModal({ open: false, request: null, mode: 'lost' })} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-white" disabled={loadingApprove}>Hủy</button>
              <button onClick={handleApprove} disabled={!selectedReplacement || loadingApprove} className={`px-4 py-2 text-white rounded-lg flex items-center gap-2 ${!selectedReplacement || loadingApprove ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                {loadingApprove ? <><Loader2 className="w-4 h-4 animate-spin"/> Đang xử lý...</> : <><CheckCircle className="w-4 h-4"/> Xác nhận Duyệt</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {rejectModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
             <div className="p-6">
                <h3 className="text-lg font-bold text-red-600 mb-4">Từ chối đơn nghỉ phép</h3>
                <textarea className="w-full border border-gray-300 rounded-lg p-3 h-32 outline-none focus:ring-2 focus:ring-red-500" placeholder="Lý do..." value={rejectReason} onChange={e => setRejectReason(e.target.value)}></textarea>
                <div className="mt-4 flex justify-end gap-3">
                    <button onClick={() => setRejectModal({ open: false, request: null })} className="px-4 py-2 border rounded-lg">Hủy</button>
                    <button onClick={handleReject} disabled={loadingReject} className="px-4 py-2 bg-red-600 text-white rounded-lg flex items-center gap-2">{loadingReject ? '...' : 'Xác nhận'}</button>
                </div>
             </div>
          </div>
        </div>
