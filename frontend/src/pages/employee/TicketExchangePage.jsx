...
import { 
  Search, Ticket, User, Train, ArrowRight, AlertCircle, RefreshCw, 
  CreditCard, Armchair, Clock, MapPin, Loader2 
} from 'lucide-react';
// import { VE_DA_DAT_DB } from '../../services/db_mock'; <--- Xóa dòng này
import '../../styles/pages/employee/TicketExchangePage.css';
...
const TicketExchangePage = () => {
  ...
  const [foundResult, setFoundResult] = useState(null);
   ...
   function calcDuration(gioDi, gioDen) {
      // gioDi, gioDen dạng "HH:mm"
      const [h1, m1] = gioDi.split(':').map(Number);
      const [h2, m2] = gioDen.split(':').map(Number);

      let start = h1 * 60 + m1;
      let end = h2 * 60 + m2;

      // Trường hợp tàu chạy qua ngày hôm sau
      if (end < start) {
         end += 24 * 60;
      }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.success) {
        setFoundResult(result.data); // data đã được format chuẩn ở Service backend
      } else {
        setErrorMsg(result.message || 'Không tìm thấy vé.');
      }
  ...
  const handleProceedExchange = () => {
    ...
    navigate('/employee/sales/exchange/search', { 
      state: {
        ...
        from: ticket.tripInfo.gaDi === "Hà Nội" ? "HN" : ticket.tripInfo.gaDi, // Logic map mã ga đơn giản
        to: ticket.tripInfo.gaDen === "TP.Hồ Chí Minh" ? "SG" : ticket.tripInfo.gaDen,
        date: new Date().toISOString().split('T')[0] 
      }
    });
  ...
  return (
    <div className="page-wrapper" style={{ paddingTop: '20px' }}>
      <div className="exchange-container">
          ...
          <>
            ...
            <div className="search-box-card">
              <div className="search-body">
                ...
                <div className="flex gap-3">
                   <button 
                      className="btn-search-lg flex-1 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed" 
                   ...
                   <button className="btn-back-outline w-32" onClick={() => navigate(-1)} disabled={loading}>Quay lại</button>
                </div>
              </div>
        ...
        {/* VIEW 2: CHI TIẾT VÉ (Data từ API) */}
        {foundResult && (
          <div className="detail-card-lg animate-fade-in border border-slate-200 shadow-sm mt-4">
            ...
            <div className="info-section mb-8">
               ...
               <div className="bg-slate-50 p-4 rounded-lg grid grid-cols-2 gap-8">
                  <div>
                     <label className="text-xs text-slate-500 block mb-1">Họ tên</label>
                     <p className="font-bold text-slate-800">{foundResult.seat.passengerName}</p>
                  </div>
                  <div>
                     <label className="text-xs text-slate-500 block mb-1">Số điện thoại</label>
                     <p className="font-bold text-slate-800">{foundResult.ticket.customerInfo?.sdt || "---"}</p>
            ...
            <div className="info-section mb-8">
               ...
               <div className="bg-slate-50 p-6 rounded-lg relative overflow-hidden">
                  <div className="flex justify-between mb-4">
                     <span className="text-xl font-bold text-slate-800">{foundResult.ticket.tripInfo.tenTau}</span>
                     <span className="bg-white border px-2 py-1 rounded text-xs font-semibold text-slate-500 shadow-sm">Đường sắt VN</span>
                  </div>
                  <div className="flex justify-between items-center">
                     <div className="text-left">
                        <span className="text-xs text-slate-500 flex items-center gap-1 mb-1"><MapPin size={12}/> Ga đi</span>
...
//                      </div> */}
                     ...
                     <div className="flex-1 px-8 text-center relative">
                        <div className="border-t-2 border-dashed border-slate-300 w-full absolute top-1/2 left-0 -z-0"></div>
                        <p className="text-xs text-slate-400 mt-2">{calcDuration(foundResult.ticket.tripInfo.gioDi, foundResult.ticket.tripInfo.gioDen)}</p>
                        <span className="bg-slate-50 px-2 text-xs text-slate-400 relative z-10">
                           <ArrowRight size={16} className="inline"/>
                        </span>
                        <p className="text-xs text-slate-400 mt-2">{new Date(foundResult.ticket.tripInfo.ngayDi).toLocaleDateString('vi-VN')}</p>
                     </div>
            ...
            <div className="info-section mb-8">
               ...
               <div className="flex justify-between items-center bg-white border border-slate-200 p-4 rounded-lg">
                  <div>
                     <span className="block font-bold text-slate-800 text-lg mb-1">
                        {foundResult.seat.tenToa} - Mã VT: {foundResult.seat.seatNum}
                     </span>
                     <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded border border-slate-200">
                        {foundResult.seat.tenToa}
                     </span>
                  </div>
                  <span className="text-lg font-bold text-slate-800">
                     {foundResult.seat.price.toLocaleString()}đ
